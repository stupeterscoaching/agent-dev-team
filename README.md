...
- [Ollama](https://ollama.com) running locally with at least one model pulled at each tier — larger models produce better results for Director and Manager tiers, smaller faster models work well for workers
- A Discord server configured with bots (see Discord Setup below)
...
1. Create a Discord server with these 4 channels:
...
5. Right-click each channel and your server name to copy IDs into your `.env` file
...
npm install
cp .env.example .env
# Fill in your tokens — see .env.example for all required values
npm start
```
...
# When the project is complete, Tech Lead notifies you in #director
...
- Local models produce functional but low quality code — quality improves significantly with frontier models at the Director and Manager tiers
- Tech Lead cannot formally approve its own PRs due to GitHub's same-account restriction — uses comment + direct merge instead
- Project names are model-generated and may vary between runs
- All agents share org-wide Discord channels — per-project channels coming in v1.1.0
